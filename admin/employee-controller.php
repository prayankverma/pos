...
if (isset($_POST['AddEmployee'])) {
    ...
    if ($result) 
    {
        ...
        if($updateResult)
        {
            echo "<script>alert('Employee details added successfully!');location.href='employee-list.php';</script>";
        }
        else
        {
             echo "<script>alert('Warning ! Employee details added successfully! But userid is not created. kindly connect with Admin Team.');location.href=employee-list.php</script>";
        }
...
if (isset($_POST['UpdateAgent'])) {
    ...
    $currentDateTime = date('Y-m-d H:i:s');
     ...
     $updateSql = "UPDATE manage_users SET status='$status', name = '$agentname', email='$email', mobile='$mobile', address='$address', district='$district',  state='$state', pincode='$pincode',  pannumber='$pannumber',  adharnumber='$adharnumber', updated_by='$userid', updated_at='$currentDateTime' WHERE id = '$id'";
        
        $updateResult = mysqli_query($con, $updateSql);
        if($updateResult)
        {
            echo "<script>alert('Agent details updated successfully!');location.href='employee-list.php';</script>";
        }
        else
        {
            echo "<script>alert('Error: " . mysqli_error($con) . "');</script>";
