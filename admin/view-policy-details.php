...
try
{
  if (isset($_POST['DocumentVerify'])) 
  {
    $newstatus="Document verified";
    ...
    if ($result) 
    {
      $updateQuery="update manage_policy set current_status='$newstatus', updated_by='$userid', updated_at='$currentDateTime' where id='$policy_id'";
      $updateResult = mysqli_query($con, $updateQuery);
      if($updateResult)
      {
          echo "<script>alert('Document verification has been verified successfully!');location.href='view-policy-details.php?token=" . $token . "';</script>";
      }
      else
      {
...
try
{
  ...
  {
    $amount=get_safe_value($con, $_POST['paymentamount']);
    $newstatus="Payment Request";
    $currentDateTime = date('Y-m-d H:i:s');
    ...
    if ($result) 
    {
      $updateQuery="update manage_policy set amount='$amount',current_status='$newstatus', updated_by='$userid', updated_at='$currentDateTime' where id='$policy_id'";
      $updateResult = mysqli_query($con, $updateQuery);
      if($updateResult)
      {
         echo "<script>alert('Payment Request has been send successfully!');location.href='view-policy-details.php?token=" . $token . "';</script>";
      }
      else
      {
           echo "<script>alert('Error! Something went wrong.')</script>";
...
try
{
  if (isset($_POST['attachpolicy'])) 
  {
    ...
    if ($result) 
    {
      $policy_file = addslashes(file_get_contents($_FILES['policy_file']['tmp_name']));

      $updateSql = "UPDATE manage_policy SET current_status='$newstatus', policy_file='$policy_file' WHERE id = '$policy_id'";
      $updateResult = mysqli_query($con, $updateSql);
      if($updateResult)
      {
        echo "<script>alert('Policy File has been uploaded successfully!');location.href='view-policy-details.php?token=" . $token . "';</script>";
                    ...
                    <div class="col-lg-12 col-md-12 mt-3">
                      <form action="" method="post" enctype="multipart/form-data">
                        <?php if($current_status=='pending'){ ?>
                          <button type="submit" name="DocumentVerify" onclick="return confirm('Are you sure you want to verify the document?')" class="btn btn-primary btn-sm">Verify Document</button>
                        <?php  } else if($current_status=='Document verified'){ ?>
                          <div class="row mb-3">
                            
                            <div class="col-md-6">
                              <input  type="number" name="paymentamount" placeholder="Enter amount" class="form-control">
                            </div>
                            <div class="col-md-6">
                              <button type="submit" name="PaymentRequest" onclick="return confirm('Are you sure you want to send payment request?')" class="btn btn-primary">Send Payment Request</button>
                            </div>
                          </div>
                        <?php } else if($current_status=='Payment Request'){ ?>  
                            <button type="submit" name="PaymentRecieved" onclick="return confirm('Are you sure payment has benn received ?')" class="btn btn-primary btn-sm">Confirm Payment Recieved?</button>
                         
                         <?php } else if($current_status=='Payment Done'){ ?>  
                          

                        <div class="col-lg-12">
                            <input type="file" name="policy_file" class="form-control" required>
                        </div>
                        <div class="col-lg-12 mt-3">
                          <button type="submit" name="attachpolicy" onclick="return confirm('Are you sure want attach policy file ?')" class="btn btn-primary btn-sm">Attach Policy</button>
                        </div>
